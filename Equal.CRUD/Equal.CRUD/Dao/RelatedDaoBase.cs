...
namespace Equal.CRUD.Dao
{
    public abstract class RelatedDaoBase<T, U> : RelatedSelectDaoBase<T, U>, IRelatedDaoBase<T, U> where T : RelatedDomainBase<U>
    {
        ...
        protected RelatedDaoBase(ISqlMapper sqlMapper) : base(sqlMapper)
        {
        }
        ...
        /// <param name="condition">领域对象，如为null则不更新</param>
        /// <returns>更新记录数</returns>
        protected virtual int UpdateRelated(string relatedDomain, string relatedDomainId, Hashtable condition)
        {
        ...
        public int UpdateRelated(string relatedDomain, string relatedDomainId, IList<U> idList)
        {
            if (idList == null || idList.Count == 0)
                return 0;
            return UpdateRelated(relatedDomain, relatedDomainId, new Hashtable() { { SqlMapConstants.IdsParam, idList } });
        }
        ...
        /// <returns>重置记录数</returns>
        public int RestoreRelated(IList<U> idList)
        {
            return UpdateRelated(string.Empty, string.Empty, idList);
        }
        ...
        /// </summary>
        /// <param name="relatedDomain">关联领域对象</param>
        /// <param name="relatedDomainList">关联领域对象Id集合</param>
        /// <param name="condition">查询条件</param>
        /// <returns>删除记录数</returns>
        public int DeleteByRelated(string relatedDomain, IList<string> relatedDomainList, ICondition condition = null)
        {
            //加载RelatedSelectDaoBase内CreateRelatedConditionHashtable方法构造查询Hashtable
            return Delete(CreateRelatedConditionHashtable(condition, relatedDomain, false, string.Empty, relatedDomainList));
        }
