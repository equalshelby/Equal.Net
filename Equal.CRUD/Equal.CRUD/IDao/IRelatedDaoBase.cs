...
namespace Equal.CRUD.IDao
{
    ...
    public interface IRelatedDaoBase<T, U> : IRelatedSelectDaoBase<T, U>, IDaoBase<T, U> where T : RelatedDomainBase<U>
    {
        ...
        /// <param name="t">领域对象</param>
        /// <returns>更新记录数</returns>
        int UpdateRelated(string relatedDomain, string relatedDomainId, T t);
        ...
        /// <param name="domainList">领域对象集合</param>
        /// <returns>重置记录数</returns>
        int RestoreRelated(IList<T> domainList);
        ...
        /// <param name="condition">查询条件</param>
        /// <returns>删除记录数</returns>
        int DeleteByRelated(string relatedDomain, IList<string> relatedDomainList, ICondition condition = null);
        #endregion
    }
}
