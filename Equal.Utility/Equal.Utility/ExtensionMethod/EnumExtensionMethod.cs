...
namespace Equal.Utility
{
    ...
    public static class EnumExtensionMethod
    {
        ...
        /// <param name="e"></param>
        /// <returns></returns>
        public static string GetStringAttribute<T>(this Enum e) where T : EnumStringAttribute//T必须继承(实现)枚举String特性
        {
            if (e == null)
                return string.Empty;
        ...
        public static string GetText(this Enum e)
        {
            if (e == null)
                return string.Empty;
            var attributes = (EnumTextAttribute[])e.GetType().GetField(e.ToString()).GetCustomAttributes(typeof(EnumTextAttribute), false);//何种情况可获得多个特性值？？（有的特性可添加多个）
            if (attributes.Count() > 0)
